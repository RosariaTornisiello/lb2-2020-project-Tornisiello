...
def split_X_y(w, profiles_path, folds):
    ...
    for filename in os.listdir(profiles_path):
        ID = filename[0:-8]
        test_fold.append(folds[ID])
        profile = pd.read_csv(profiles_path + filename, sep='\t')
        profile = padding(w, profile).to_numpy()
        profile_X, profile_y = generate_x_y(w, profile)
        X_train = X_train + profile_X
        y_train = y_train + profile_y
...
#print(X_train.shape, y_train.shape, test_fold.shape)

my_scorer = make_scorer(myMCC(y_true, y_pred))
ps = PredefinedSplit(test_fold)
mySVC = SVC(C=4, kernel='rbf', gamma=0.5, random_state=42)
cv_results = cross_validate(mySVC, X_train, y_train, scoring= my_scorer, cv=ps, return_estimator=True, n_jobs=5)
estimators = cv_results['estimator']

#joblib.dump(estimators, '/home/rosaria/Desktop/LAB2/LAB2_project/SVM/myCV_estimators')
#models = joblib.load('/home/rosaria/Desktop/LAB2/LAB2_project/SVM/myCV_estimators')
